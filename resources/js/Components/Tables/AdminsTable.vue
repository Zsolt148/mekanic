            ...
            <v-col>
                ...
                >
                    ...
                    <template v-slot:item.email="{ item }">
                        <span class="table--field">{{ item.email }}</span>
                        <p class="table--small">+36 30 123 4567</p>
                    </template>
    
                    <template v-slot:item.status="{ item }">
                        <div v-if="!item.trashed">
                            <span class="table--field">{{ item.statusText }}</span><br>
                            <small class="table--small">{{ dateFormat(item.statusDate) }}</small>
                        </div>
                        <div v-else>
                            <span style="color:#BA1B1B!important" class="table--field">{{ trans('Deleted') }}</span><br>
                            <small style="color:#BA1B1B!important" class="table--small">{{ dateFormat(item.deleted_at) }}</small>
                        </div>
                    </template>
    
                    <template v-slot:item.rolesText="{ item }">
                        <span class="table--field">{{ item.rolesText }}</span>
                    </template>
    
                    <template v-slot:item.last_login_at="{ item }">
                        <span class="table--field">{{ item.last_login_at }}</span>
                    </template>
                        ...
                        >
                            ...
                            <v-list dense>
                                ...
                                <v-list-item v-else-if="item.trashed" @click="restoreUser(item)">
                                    <v-list-item-icon>
                                        <v-icon>mdi-restore-outline</v-icon>
                                    </v-list-item-icon>
                                    <v-list-item-content>
                                            {{ trans('Restore') }}
                                    </v-list-item-content>
    ...
    data: function () {
        return {
            ...
            headers: [
                ...
                { text: this.trans("Titles"), value: "rolesText" },
